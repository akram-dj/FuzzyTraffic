...
def optimize_membership_with_ga(generations: int = 12, pop_size: int = 16, seed: int = 42, verbose: bool = False) -> FuzzyParams:
    random.seed(seed)
    np.random.seed(seed)
    ...
    for gen in range(generations):
        ...
        if verbose:
            print(f"[GA] generation {gen + 1}/{generations} best_mse={_fitness(ranked[0], samples):.4f}")
