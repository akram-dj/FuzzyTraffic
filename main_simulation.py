...
def _accumulate_waiting_seconds(conn: traci.connection.Connection) -> float:
    # Snapshot waiting veh*sec for this step (integrated over all steps).
    return float(sum(1 for veh_id in conn.vehicle.getIDList() if conn.vehicle.getSpeed(veh_id) < 0.1))


def _accumulate_timeloss_seconds(conn: traci.connection.Connection) -> float:
    # SUMO metric: cumulative delay relative to free-flow speed for active vehicles at this step.
    return float(sum(conn.vehicle.getTimeLoss(veh_id) for veh_id in conn.vehicle.getIDList()))
...
def run_parallel_comparison(use_gui: bool = True) -> None:
    ...
    common_args = ["-c", "config.sumocfg", "--start", "true", "--quit-on-end", "true", "--seed", "42"]
    ...
    total_timeloss_fuzzy = 0.0
    total_timeloss_standard = 0.0
    ...
    while conn_fuzzy.simulation.getMinExpectedNumber() > 0 or conn_standard.simulation.getMinExpectedNumber() > 0:
        if conn_fuzzy.simulation.getMinExpectedNumber() > 0:
            ...
            total_timeloss_fuzzy += _accumulate_timeloss_seconds(conn_fuzzy)
        ...
        if conn_standard.simulation.getMinExpectedNumber() > 0:
            ...
            total_timeloss_standard += _accumulate_timeloss_seconds(conn_standard)
    ...
    print(f"Fuzzy integrated timeLoss (s):         {total_timeloss_fuzzy:.1f}")
    print(f"Standard integrated timeLoss (s):      {total_timeloss_standard:.1f}")
